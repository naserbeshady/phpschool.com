...
class crGantt extends crChart {
	...
	function SetupStartEnd() {
		$sql = "SELECT MIN(" . ewr_QuotedName($this->TaskStartField, $this->TaskTableDBID) . "), MAX(". ewr_QuotedName($this->TaskEndField, $this->TaskTableDBID) . ") FROM " . ewr_QuotedName($this->TaskTable, $this->TaskTableDBID);
...
//			} elseif ($interval == EWR_GANTT_INTERVAL_MONTH) {
	...
	function OutputDataTable() {
		...
		$sql = "SELECT * FROM " . ewr_QuotedName($this->ProcessTable, $this->ProcessTableDBID);
	...
	function OutputTasks() {
		...
		$sql = "SELECT * FROM " . ewr_QuotedName($this->TaskTable, $this->TaskTableDBID);
		if ($this->TaskFilter <> "") $sql .= " WHERE " . $this->TaskFilter;
	...
	function OutputProcesses() {
		...
		if ($this->ProcessTable <> "") { // Use process table
			...
			$sql = "SELECT DISTINCT " . ewr_QuotedName($fid, $this->TaskTableDBID) . " AS id, " .
				ewr_QuotedName($fname, $this->TaskTableDBID) . " AS name, " . ewr_QuotedName($fstart, $this->TaskTableDBID) . " FROM " . ewr_QuotedName($this->TaskTable, $this->TaskTableDBID);
	...
	function Xml() {
		...
		if ($this->MilestoneTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->MilestoneTable, $this->MilestoneTableDBID);
			...
			$sql = "SELECT " . ewr_QuotedName($this->TaskIdField, $this->TaskTableDBID) . " AS ". ewr_QuotedName('taskId', $this->TaskTableDBID) . ", " .
				...
				" FROM " . ewr_QuotedName($this->TaskTable, $this->TaskTableDBID) .
		...
		if ($this->TrendlineTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->TrendlineTable, $this->TrendlineTableDBID);
		...
		if ($this->ConnectorTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->ConnectorTable, $this->ConnectorTableDBID);
			...
			$sql = "SELECT " . ewr_QuotedName($this->TaskFromTaskIdField, $this->TaskTableDBID) . " AS ". ewr_QuotedName('fromTaskId', $this->TaskTableDBID) . ", " .
				ewr_QuotedName($this->TaskIdField, $this->TaskTableDBID) . " AS ". ewr_QuotedName('toTaskId', $this->TaskTableDBID) . " FROM " . ewr_QuotedName($this->TaskTable, $this->TaskTableDBID);
